...
def main() -> None:
    ...
    # Latency + throughput-vs-batch for both models (full pipeline, beam=1)
    print(f"{'model':6} {'lat p50':>8} {'p95':>7} {'p99':>7} | throughput req/s by batch")
    print("-" * 78)
    for model in ["mini", "tiny"]:
    ...
    # Beam sweep (mini, batch=16) -- the quality/throughput trade
    print(f"\nBeam sweep (mini, batch=16):")
    for beam in [1, 2, 4, 5]:
        pipe = build_pipeline(model="mini", beam_size=beam)
        rps = median_throughput(pipe, 16, args.total, args.runs)
        print(f"  beam={beam}: {rps:.0f} req/s")
        rows.append({"metric": "beam_throughput", "model": "mini", "beam": beam, "batch": 16, "value": round(rps, 1)})
